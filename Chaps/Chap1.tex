...
下面我们将介绍一些重要的定义。
一个$N\times M$矩阵 $\mbf A$的\emph{转置共轭矩阵}(adjoint)\footnote{译者注：本书先前的版本中 adjoint 曾译为"伴随矩阵"，此处指矩阵的转置共轭（即各元素取复共轭后再转置）。注意这与线性代数中另一种"伴随矩阵"（adjugate，即由代数余子式构成的矩阵）含义不同。}，
...
\begin{enumerate}
 ...
 \item 若矩阵$\mbf A$的逆矩阵与其转置共轭矩阵相等，则$\mbf A$为\emph{幺正}矩阵：
 \begin{equation}
     {\mbf A}^{-1} = \madj{A}
     \label{eq:1.36}
 \end{equation} 
 ...
 \item 转置共轭矩阵 等于自身(self-adjoint)的矩阵\footnote
	 {译者注：如果直接按英文翻译的话，
	 此处"自伴矩阵"似乎更能与英文对应。但为了顾名思义，
	 译文中换了一种表述。}
 称为\emph{Hermitian}矩阵 ，即：
 \begin{subequations}
     \begin{equation}
     \madj{A} = {\mbf A} \\
     \label{eq:1.37a}
...
\autoref{eq:1.27}提到，
一个列矩阵$\mbf a$的转置共轭矩阵$\madj{a}$是一个行矩阵：
\begin{equation}
...
算符$\op{O}$的\emph{转置共轭}算符标记为$\oadj{O}$。
如果算符$\op{O}$作用在右矢$\ket{a}$上将之转变成$\ket{b}$，
则其转置共轭算符将左矢$\bra{a}$转变成$\bra{b}$：
\begin{equation}
 \bra{a}\oadj{O} = \bra{b}
 \label{eq:1.57}
\end{equation}
上式与\autoref{eq:1.52}互为转置共轭关系。
...
\end{equation}
由于$a, b$和$c$为任意矢量，
这样我们就证明了算符$\oadj{O}$的矩阵表示与算符$\op{O}$的矩阵表示的转置共轭矩阵相等：
\begin{equation}
...
\begin{equation}
 ...
 \label{eq:1.65}
\end{equation}
上式中我们用到了\autoref{eq:1.49}和转置共轭矩阵的定义，
从而得到如下关系：
\begin{equation}
