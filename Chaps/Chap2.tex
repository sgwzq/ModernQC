...
现在引进\emph{湮灭算符} $a_i$. 
它是产生算符$\cs_i$的转置共轭算符(adjoint operator)(即$(\cs_i)^\dagger = a_i$). 
...
那么, 
为什么湮灭算符被定义为产生算符的转置共轭算符？考虑如下行列式
\begin{align}
\ket{K} = \ket{\chi_i\chi_j}
\end{align}
很明显
...
效果和湮灭算符相同. 
举个例子, 
\autoref{eq:2.205}的转置共轭就是
\begin{align}
\bra{K}\cs_i = \bra{\chi_j}
\end{align}

为得到湮灭算子的反对易关系, 
我们取\autoref{eq:2.194}的转置共轭. 
由于(见\autoref{ex:1.3})：
\begin{align}
